...
type SeriesQuerier interface {
	GetSeriesByID(id string) (types.Series, error)
	GetSeriesMovies(id, userID string) ([]types.SeriesMovies, error)
}
...
func (r *SeriesRepository) GetSeriesByID(id string) (types.Series, error) {
	var series types.Series
	err := r.db.Get(&series, seriesByIDQuery, id)
	return series, err
}

func (r *SeriesRepository) GetSeriesMovies(id, userID string) ([]types.SeriesMovies, error) {
	var movies []types.SeriesMovies
	err := r.db.Select(&movies, seriesMoviesQuery, id, userID)
	return movies, err
}
