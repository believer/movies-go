...
;(function () {
	...
	function appendNode(newNode) {
		...
		document.head.appendChild(newElt)
		if (newNode._preloadHint)
			newElt.addEventListener("load", () => newNode._preloadHint.remove(), {
				once: true,
			})
		return null
	}
	...
	async function mergeHead(newContent, defaultMergeStrategy) {
		if (newContent && newContent.indexOf("<head") > -1) {
			...
			// extract head tag
			let headTag = contentWithSvgsRemoved.match(
				/(<head(\s[^>]*>|>)([\s\S]*?)<\/head>)/im
			)
			...
			if (headTag) {
				...
				let mergeStrategy =
					api.attributeValue(newHeadTag, "hx-head") || defaultMergeStrategy
				...
				for (const currentHeadElt of currentHead.children) {
					...
					let isPreserved =
						api.attributeValue(currentHeadElt, "hx-preserve") === "true"
					if (inNewContent || isPreserved) {
						...
						if (mergeStrategy === "append") {
							...
							if (
								htmx.trigger(document.body, "htmx:before:head:remove", {
				...
				for (const newNode of nodesToAppend) {
					if (newNode.tagName === "SCRIPT" && newNode.defer) {
						...
						log("adding: ", newNode)
						if (
							htmx.trigger(document.body, "htmx:before:head:add", {
								headElement: newNode,
							}) !== false
						) {
